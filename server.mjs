...
app.use(function (req, res, next) {
  ...
  if(!req.cookies.Token){
    res.status(401).send({message: "include http-only Credentiols with every request"})
    return;
  }
  jwt.verify(req.cookies.Tokeen,SECRET,function(err,decodedData){
    if(!err){
      console.log("decodedData:",decodedData);
      const nowDate = new Date().getTime() / 1000;
     
      if(decodedData.exp < nowDate){ //after expire 5 min (in milis)
